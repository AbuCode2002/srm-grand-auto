            ...
            <div class="table-checkable table-highlight-head table-responsive">
              ...
              <table v-if="roleUser === 1" role="table" aria-busy="false" aria-colcount="5"
                ...
                <tr v-for="item in order" :value="item" role="row" class="">
                  <td v-if="item.status.name === 'Новая заявка'" aria-colindex="1" role="cell" class="mb-4">
                  ...
                  <td aria-colindex="8" role="cell" class="">{{ item.status ? item.status.name : '-' }}</td>
              ...
              <table v-if="roleUser === 2" role="table" aria-busy="false" aria-colcount="5"
                ...
                <tr v-for="item in order" :value="item" role="row" class="">
                  ...
                  <td aria-colindex="7" role="cell" class="">{{ item.status ? item.status.name : '-' }}</td>
              ...
              <table v-if="roleUser === 4" role="table" aria-busy="false" aria-colcount="5"
                ...
                <tr v-for="item in order" :value="item" role="row" class="">
                  <td aria-colindex="1" role="cell" class="mb-4">
                      ...
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Выберите дату для проведения диагностики</h5>
                            <button type="button" data-dismiss="modal" data-bs-dismiss="modal" aria-label="Close" class="btn-close"></button>
                          </div>

                          <div class="col-xl-12">
                            <div class="">
                              <input v-model="date" type="text" id="date" class="form-control" v-maska="'##-##-####'" placeholder="__/__/____" />
                            </div>
                          </div>
                          ...
                          <div class="modal-footer">
                            ...
                            <button @click.prevent="postDate(item.id)" type="button" class="btn btn-primary"
                    ...
                    <button v-if="item.status.name === 'ДА ожидает согласования в отделе по работе с партнерами'"
                            @click.prevent="pushToUpdateDefectiveAct(item.id)"
                            class="btn btn-success mb-2 me-1">
                      {{ item.id }}
                    </button>
                    <button v-if="item.status.name === 'ДА на согласовании в отделе по работе с клиентами'"
                            @click.prevent="pushToUpdateDefectiveAct(item.id)"
                            class="btn btn-success mb-2 me-1">
                      {{ item.id }}
                    </button>
                    <button v-if="item.status.name === 'Согласован'" @click.prevent="pushToUpdateDefectiveAct(item.id)"
                            class="btn btn-success mb-2 me-1">
                      {{ item.id }}
                    </button>
                  ...
                  <td aria-colindex="10" role="cell" class="">{{ item.mileage }}</td>
                  <td v-if="item.is_broken === 1" aria-colindex="11" role="cell" class="">Исправна</td>
                  <td v-else aria-colindex="11" role="cell" class="">Не исправна</td>
                  <td aria-colindex="12" role="cell" class="">{{ item.created_at }}</td>
