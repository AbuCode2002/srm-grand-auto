    ...
    public function store(DefectiveActData $data, Order $order): DefectiveAct
    {
        ...
        if ($defectiveActForOrder->isEmpty()) {
            ...
            foreach (array_keys($data->service) as $item) {
                ...
                $service->service_name_id = $data->service[$item]["serviceName"]["name"]["id"] ?? 0;
            ...
            foreach (array_keys($data->service) as $item1) {
                ...
                if ($service) {
                    ...
                    $service->service_name_id = ServiceName::query()
                        ->where('name', $data->service[$item1]["name"]["name"])
                        ->value('id') ?? 0;
                ...
                foreach (array_keys($data->spare_parts[$item1]) as $value) {
                    ...
                    if ($part) {
                        $part->part_name_id = PartName::query()
                            ->where('name', $data->spare_parts[$item1][$value]["name"]["name"])
                            ->value('id');
                        ...
                        $partModel->part_name_id = PartName::query()
                            ->where('name', $data->spare_parts[$item1][$value]["name"]["name"])
                            ->value('id');
